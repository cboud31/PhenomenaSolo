...
const apiSetup = async () => {
  ...
  const reportsCreatedDirectly = await Promise.all(
    reportsToCreate.map(async ({ title, location, description, password }) => {
      const {
        rows: [reportCreated],
      } = await client.query(
        `
...
describe('API', () => {
  const reportToPost = {
    ...
    description: 'the little fellow put on a ring, and i swear he disappeared',
    password: 'FrodoIsMysterious',
  };
  const commentFieldsToPost = {
  ...
  describe('server', () => {
    ...
    it('Responds to requests', async () => {
      await expect(axios.get(`${API_URL}/foo-bar`)).rejects.toThrow(
  ...
  describe('GET request for /api/reports', () => {
    ...
    it('reports should reflect those in the database', async () => {
      expect(singleReportResponse).toEqual(
        expect.objectContaining({
      ...
      expect(singleReportResponse.description).toEqual(
        singleReportQueried.description
      );
      expect(singleReportResponse.title).toEqual(singleReportQueried.title);
      expect(singleReportResponse.location).toEqual(
        singleReportQueried.location
      );
    });
  });
  describe('POST request for /api/reports', () => {
    ...
    it('on caught error, call next(error), which sends back a 500 error', async () => {
      await expect(
        axios.post(`${API_URL}/api/reports`, { nothing: undefined })
  ...
  describe('POST request for /api/reports/:reportId/comments', () => {
    ...
    it('on caught error, call next(error), which sends back a 500 error', async () => {
      await expect(
        axios.post(`${API_URL}/api/reports/${2300}/comments`, {
          nothing: undefined,
        })
      ).rejects.toThrow('Request failed with status code 500');
    });
    it('on success, it should send back the object returned by createReportComment', async () => {
      const { data, status } = await axios.post(
        `${API_URL}/api/reports/${postedReportResponse.id}/comments`,
        commentFieldsToPost
      );
  ...
  describe('DELETE request for /api/reports/:reportId', () => {
    ...
    it('on caught error, call next(error), which sends back a 500 error', async () => {
      await expect(
        axios.delete(`${API_URL}/api/reports/${2300}`)
      ).rejects.toThrow('Request failed with status code 500');
    });
